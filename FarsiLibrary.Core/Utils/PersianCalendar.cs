...
public sealed class PersianCalendar : Calendar
{
    ...
    {
        GetYearAndRemainingDays(validate, time, out _, out var days);
        return days;
    }
        ...
        return month switch
            {
                < 7 => 31,
                < 12 => 30,
                _ => IsLeapYear(false, year, 0) ? 30 : 29
            };
    }
    ...
    {
        GetYearAndRemainingDays(validate, time, out var year, out _);
        return year;
    }
    ...
    public static int GetCentury(DateTime time)
    {
    ...
    /// <remarks>In the Persian calendar leap years are applied every 4 or 5 years according to a certain pattern that iterates in a 2820-year cycle. A common year has 365 days and a leap year has 366 days.</remarks>
    public static int NumberOfLeapYearsUntil(int year)
    {
